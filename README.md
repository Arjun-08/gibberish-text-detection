...
## Version 2 — Building the Benchmark

Version 2 established the complete experimental framework.

The goal was to create a controlled dataset containing four types of text:
...
# 4. Version 3 — Making the Benchmark Rigorous

Version 3 strengthened the dataset-generation process.
...
The final V3 common-test evaluation produced the following results.
...
│
├── artifacts/
│   ├── statistical_ngram.joblib
│   └── classical_tfidf_logreg.joblib
│
├── results/
...
The dataset generator uses a fixed random seed:
...
The same held-out test set is then used for all three models.

This makes it possible to reproduce the benchmark and compare future models against the same baseline.
...
The long-term architecture is a **Self-Healing RAG ingestion pipeline**.
...
```text
            ...
            Clean           Suspicious
              │                 │
              ▼                 ▼
          Chunking        Alternate Parser
              │                 │
              ▼                 ▼
         Embeddings       Re-extraction
              │                 │
              ▼                 │
         Vector Store ◄─────────┘
              │
...
The detector becomes one component in a larger system that can **detect extraction failures, recover from them, and continue ingestion automatically**.
...
```text
Model A
From-scratch Character 3-Gram Language Model
